 ...
 */
class CustomContentTranslationController extends ContentTranslationController {
...
//    return parent::overview( $route_match, $entity_type_id );
    ...
    if ($this->languageManager()->isMultilingual()) {
      ...
      $custom_keys = ['langType', 'region', 'countryName', 'availableCurrency', 'regionShort', 'polarCountryCode', ];

      $extra_fields = array();
      $configurable_languages = \Drupal::entityTypeManager()->getStorage('configurable_language')->loadByProperties( [] );
      foreach($configurable_languages as $fid => $configurable_language) {
        $extra = $configurable_language->getThirdPartySettings('shp_tilepage');
        $extra_fields[$fid] = $extra;
      }
      ...
      foreach ($languages as $language) {
        ...
        $custom_string = '' ;
        /*if ( $configurable_language = $configurable_languages[$langcode] ) {

              $custom_string = ' [ ' ;
              $custom_settings = $configurable_language->getThirdPartySettings('shp_tilepage');
              foreach($custom_keys as $custom_key) {
                 $val = isset($custom_settings[$custom_key]) ? $custom_settings[$custom_key] : '';
                 if ( is_array($val) )  $val = implode(', ' , $val) ;
                 $custom_settings[$custom_key] = $val ;
                 $custom_string .= ' (' . $custom_key . ': ' . $val . ' ) ' ;
              }
              $custom_string .= ' ] ' ;        
//              $language_name .= $custom_string;  
        } */
        ...
        $extras = isset($extra_fields[$langcode]) ? $extra_fields[$langcode] : array();
        $extra_type = ( isset($extras['langType']) && is_string($extras['langType']) )   ? $extras['langType'] : '';
        $extra_region = ( isset($extras['region']) && is_string($extras['region']) )   ? $extras['region'] : '';
        $extra_currency = ( isset($extras['availableCurrency']) && is_string($extras['availableCurrency']) )   ? $extras['availableCurrency'] : '';

//  $language_name, $row_title,$source_name,$status,$operations,

        if ($show_source_column) {
          $rows[] = array(
            [ 'data' => $language_name, 'field' => 'Name', ],    //  1
            [ 'data' => $extra_type, 'field' => 'Type', ],    //  2
            [ 'data' => $extra_region, 'field' => 'Region', ],    // 3
            [ 'data' => $extra_currency, 'field' => 'Currency', ],    // 4
            [ 'data' => $row_title,  ],    // 5
            [ 'data' => $source_name, ],    // 6
            [ 'data' => $status, ],    // 7
            [ 'data' => $operations,],    // 8
          ...
          $rows[] = array(
                      [ 'data' => $language_name, 'field' => 'Name', ],
                      [ 'data' => $extra_type, 'field' => 'Type', ],
                      [ 'data' => $extra_region, 'field' => 'Region', ],
                      [ 'data' => $extra_currency, 'field' => 'Currency', ],
                      [ 'data' => $row_title,  ],
                      [ 'data' => $status, ],
                      [ 'data' => $operations,],
                      );
    ...
    $header = [
      ...
      [  'data' => t('Region'), 'field' => 'Region', ],
      [  'data' => t('Currency'), 'field' => 'Currency', ],
      [  'data' => t('Translation'), ],
...
*/
    ...
    $build['content_translation_overview'] = array(
     ...
     $build['#attached']['library'][] = 'core/drupal.tableresponsive';
     $build['#attributes']['class'][] = 'responsive-enabled';

     $build['content_translation_overview']['#attached']['library'][] = 'core/drupal.tableresponsive';
     $build['content_translation_overview']['#attributes']['class'][] = 'responsive-enabled';
  ...
  public function add(LanguageInterface $source, LanguageInterface $target, RouteMatchInterface $route_match, $entity_type_id = NULL) {
    ...
    $test =  $this->entityFormBuilder()->getForm($entity, $operation, $form_state_additions);
    return $test;
  ...
  public function access($operation = 'view', Drupal\Core\Session\AccountInterface $account = NULL, $return_as_object = false) {
