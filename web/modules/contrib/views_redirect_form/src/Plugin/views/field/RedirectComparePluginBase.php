...
abstract class RedirectComparePluginBase extends FieldPluginBase {
  ...
  /**
   * Calculates a form key.
   *
   * This generates a key that is used as the checkbox return value when
   * submitting a form. This key allows the entity for the row to be loaded
   * totally independently of the executed view row.
   *
   * @param \Drupal\Core\Entity\EntityInterface $entity
   *   The entity to calculate a form key for.
   *
   * @return string
   *   The form key representing the entity's id, language and revision (if
   *   applicable) as one string.
   *
   * @see self::loadEntityFromDiffFormKey()
   */
  protected function calculateEntityFormKey(EntityInterface $entity) {
  ...
  /**
   * Loads an entity based on a form key.
   *
   ...
   *   as part of the form key.
   */
  protected function loadEntityFromFormKey($form_key) {
    $key = base64_decode($form_key);
