...
class ModerationInformation implements ModerationInformationInterface {
  ...
  /**
   * Returns the state of the forward revision if there is one.
   * @param  ContentEntityInterface $entity
   *
   * @return string
   */
  public function forwardRevisionState(ContentEntityInterface $entity) {
    if ($this->hasForwardRevision($entity)) {
      $latest_revision = $this->getLatestRevision($entity->getEntityTypeId(), $entity->id());
      return $latest_revision->moderation_state->entity->label();
    }
    return NULL;
  }
